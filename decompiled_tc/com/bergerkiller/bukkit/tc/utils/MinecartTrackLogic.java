...
public class MinecartTrackLogic {
    ...
    public MinecartTrackLogic(Block block) {
        this(block.getWorld(), new IntVector3(block), WorldUtil.getBlockData((Block)block));
    }
    ...
    private void getNeighbours(Rails rails) {
        ...
        if (rails.isOnSlope()) {
            ...
            for (BlockFace face : FaceUtil.getFaces((BlockFace)rails.getDirection())) {
    ...
    private MinecartTrackLogic createTrackLogic(IntVector3 blockposition) {
        BlockData iblockdata = WorldUtil.getBlockData((World)this.world, (IntVector3)blockposition);
        if (MaterialUtil.ISRAILS.get(iblockdata).booleanValue()) {
        ...
        iblockdata = WorldUtil.getBlockData((World)this.world, (IntVector3)blockposition1);
        if (MaterialUtil.ISRAILS.get(iblockdata).booleanValue()) {
    ...
    private boolean isNeighbourAt(IntVector3 blockposition) {
        for (int i = 0; i < this.neigh.size(); ++i) {
            ...
            if (blockposition1.x != blockposition.x || blockposition1.z != blockposition.z) continue;
            return true;
        }
        return false;
    }
    ...
    public int getNeighbourCount() {
        ...
        for (BlockFace face : FaceUtil.AXIS) {
            if (!this.isRailsAtUpDown(this.pos.add(face))) continue;
            ++i;
        }
        return i;
    }
    ...
    private void c(MinecartTrackLogic blockminecarttrackabstract_minecarttracklogic) {
        ...
        this.data = BlockData.fromMaterialData((MaterialData)newRails);
    ...
    public MinecartTrackLogic update(boolean isPowered, boolean forcePhysics) {
        ...
        this.data = BlockData.fromMaterialData((MaterialData)newRails);
        if (forcePhysics || !WorldUtil.getBlockData((World)this.world, (IntVector3)this.pos).equals((Object)this.data)) {
            TrainCarts.plugin.setBlockDataWithoutBreaking(this.pos.toBlock(this.world), this.data);
            for (int i = 0; i < this.neigh.size(); ++i) {
                MinecartTrackLogic blockminecarttrackabstract_minecarttracklogic = this.createTrackLogic(this.neigh.get(i));
                if (blockminecarttrackabstract_minecarttracklogic == null) continue;
                blockminecarttrackabstract_minecarttracklogic.refreshNeighbours();
                if (!blockminecarttrackabstract_minecarttracklogic.isSpecialNeighbour(this)) continue;
