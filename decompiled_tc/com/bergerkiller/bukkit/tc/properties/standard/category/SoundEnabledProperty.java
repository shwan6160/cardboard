...
public final class SoundEnabledProperty
extends FieldBackedStandardTrainProperty<Boolean> {
    @CommandTargetTrain
    @PropertyCheckPermission(value="soundenabled")
    @Command(value="train soundenabled|sound <enabled>")
    @CommandDescription(value="Sets whether the train makes sound while moving")
    private void setProperty(CommandSender sender, TrainProperties properties, @Argument(value="enabled") boolean enabled) {
        properties.set(this, enabled);
        this.getProperty(sender, properties);
    }

    @Command(value="train soundenabled|sound")
    @CommandDescription(value="Displays whether the train makes sound while moving")
    private void getProperty(CommandSender sender, TrainProperties properties) {
        sender.sendMessage(ChatColor.YELLOW + "Train makes sound while moving: " + Localization.boolStr(properties.get(this)));
    }

    @PropertyParser(value="sound|soundenabled|minecartsound")
