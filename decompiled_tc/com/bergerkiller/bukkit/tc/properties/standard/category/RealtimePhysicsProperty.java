...
public class RealtimePhysicsProperty
extends FieldBackedStandardTrainProperty<Boolean> {
    @CommandTargetTrain
    @PropertyCheckPermission(value="realtime")
    @Command(value="train realtime <enabled>")
    @CommandDescription(value="Sets whether the train updates in realtime, adjusting for server tick lag and jitter")
    ...
    @Command(value="train realtime")
    @CommandDescription(value="Displays whether the train updates in realtime, adjusting for server tick lag and jitter")
    private void getProperty(CommandSender sender, TrainProperties properties) {
        sender.sendMessage(ChatColor.YELLOW + "Train updates in realtime: " + Localization.boolStr(properties.get(this)));
    }

    @PropertyParser(value="realtime")
