...
public class DebugToolUtil {
    ...
    private static final ChatColor[] chatcolor_wheel_values = new ChatColor[]{ChatColor.GREEN, ChatColor.BLUE, ChatColor.AQUA, ChatColor.LIGHT_PURPLE, ChatColor.GOLD, ChatColor.DARK_RED, ChatColor.DARK_GREEN, ChatColor.DARK_BLUE, ChatColor.DARK_AQUA, ChatColor.DARK_PURPLE, ChatColor.YELLOW, ChatColor.BLACK, ChatColor.DARK_GRAY, ChatColor.GRAY, ChatColor.WHITE};
    private static final Color[] color_wheel_values = new Color[]{Color.GREEN, Color.BLUE, Color.AQUA, Color.FUCHSIA, Color.YELLOW, Color.MAROON, Color.OLIVE, Color.NAVY, Color.TEAL, Color.PURPLE, Color.YELLOW, Color.BLACK, Color.ORANGE, Color.GRAY, Color.WHITE};
    ...
    public static String coordinates(RailPath.Position position) {
        return DebugToolUtil.coordinates(MathUtil.floor((double)position.posX), MathUtil.floor((double)position.posY), MathUtil.floor((double)position.posZ));
    }
    ...
    public static void showFaceParticles(Player viewer, Color color, double x1, double y1, double z1, double x2, double y2, double z2) {
        DebugToolUtil.showLineParticles(viewer, color, x1, y1, z1, x2, y1, z1);
        DebugToolUtil.showLineParticles(viewer, color, x1, y1, z1, x1, y2, z1);
        DebugToolUtil.showLineParticles(viewer, color, x1, y1, z1, x1, y1, z2);
        DebugToolUtil.showLineParticles(viewer, color, x1, y2, z2, x2, y2, z2);
        DebugToolUtil.showLineParticles(viewer, color, x2, y1, z2, x2, y2, z2);
        DebugToolUtil.showLineParticles(viewer, color, x2, y2, z1, x2, y2, z2);
    }
    ...
    public static void showCubeParticles(Player player, Color color, double x1, double y1, double z1, double x2, double y2, double z2) {
        DebugToolUtil.showFaceParticles(player, color, x1, y1, z1, x2, y1, z2);
        DebugToolUtil.showFaceParticles(player, color, x1, y2, z1, x2, y2, z2);
        DebugToolUtil.showLineParticles(player, color, x1, y1, z1, x1, y2, z1);
        DebugToolUtil.showLineParticles(player, color, x2, y1, z1, x2, y2, z1);
        DebugToolUtil.showLineParticles(player, color, x1, y1, z2, x1, y2, z2);
        DebugToolUtil.showLineParticles(player, color, x2, y1, z2, x2, y2, z2);
    }

    public static void showLineParticles(Player viewer, Color color, Vector p1, Vector p2) {
        DebugToolUtil.showLineParticles(viewer, color, p1.getX(), p1.getY(), p1.getZ(), p2.getX(), p2.getY(), p2.getZ());
    }
    ...
    public static void showLineParticles(Player viewer, Color color, double x1, double y1, double z1, double x2, double y2, double z2) {
        ...
        int n = MathUtil.ceil((double)(dist / 0.2));
        double dx = x2 - x1;
        double dy = y2 - y1;
        double dz = z2 - z1;
        ...
        for (int i = 0; i < n; ++i) {
            ...
            PlayerUtil.spawnDustParticles((Player)viewer, (Vector)position, (Color)color);
        }
    }
    ...
    public static void showEndOfTheRail(Player player, TrackWalkingPoint walker, double initialDistance) {
        ...
        if (Math.abs(p.posX - Math.floor(p.posX)) < 0.01 || Math.abs(p.posY - Math.floor(p.posY)) < 0.01 || Math.abs(p.posZ - Math.floor(p.posZ)) < 0.01) {
            p.move(-0.01);
        }
        if (walker.failReason == TrackWalkingPoint.FailReason.NAVIGATION_ABORTED) {
            player.sendMessage(ChatColor.RED + "A blocker sign at " + ChatColor.YELLOW + DebugToolUtil.coordinates(p) + ChatColor.RED + " is blocking trains after " + (int)(walker.movedTotal + initialDistance) + " blocks!");
        } else {
            player.sendMessage(ChatColor.RED + "End of the rail at " + DebugToolUtil.coordinates(p) + " after " + (int)(walker.movedTotal + initialDistance) + " blocks!");
        }
    }
