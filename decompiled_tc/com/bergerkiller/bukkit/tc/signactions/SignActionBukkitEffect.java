...
public class SignActionBukkitEffect
extends TrainCartsSignAction {
    public static Effect parse(SignActionEvent event) {
        ...
        try {
            if (args.length >= 1) {
                eff.pitch = (float)ParseUtil.parseDouble((String)args[0], (double)1.0);
            }
            if (args.length == 2) {
                eff.volume = (float)ParseUtil.parseDouble((String)args[1], (double)1.0);
            }
        }
    ...
    public void execute(SignActionEvent info) {
        ...
        Effect eff = SignActionBukkitEffect.parse(info);
        ...
        if (player) {
            if (info.isTrainSign() && info.isAction(SignActionType.REDSTONE_ON, SignActionType.GROUP_ENTER)) {
                for (MinecartMember<?> member : info.getGroup()) {
                    for (Player p : ((CommonMinecart)member.getEntity()).getPlayerPassengers()) {
                        eff.play(p);
                    }
                }
            } else if (info.isCartSign() && info.isAction(SignActionType.REDSTONE_ON, SignActionType.MEMBER_ENTER)) {
                for (Player p : ((CommonMinecart)info.getMember().getEntity()).getPlayerPassengers()) {
                    eff.play(p);
                }
            }
            return;
    ...
    public boolean build(SignChangeActionEvent event) {
        ...
        SignBuildOptions opt = SignBuildOptions.create().setPermission(Permission.BUILD_EFFECT).setName(event.isCartSign() ? "cart Bukkit effect player" : "train Bukkit effect player").setTraincartsWIKIHelp("TrainCarts/Signs/Effect");
