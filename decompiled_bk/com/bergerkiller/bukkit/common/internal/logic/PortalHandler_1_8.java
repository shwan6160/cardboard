...
extends PortalHandler {
    ...
    private Set<Player> _ignorePortalEventPlayers = new HashSet<Player>();
    private RunOnceTask _ignorePortalEventPlayersCleanup;

    @Override
    public void enable(CommonPlugin plugin) {
        this._ignorePortalEventPlayersCleanup = RunOnceTask.create((Plugin)plugin, () -> this._ignorePortalEventPlayers.clear());
        plugin.register(new Listener(){

            @EventHandler(priority=EventPriority.MONITOR)
            public void onEntityPortalEnter(EntityPortalEnterEvent event) {
                Player player;
                if (event.getEntity() instanceof Player && ServerPlayerHandle.fromBukkit(player = (Player)event.getEntity()).isViewingCredits()) {
                    PortalHandler_1_8.this._ignorePortalEventPlayers.add(player);
            ...
            public void onPortalEvent(PlayerPortalEvent event) {
                if (PortalHandler_1_8.this._ignorePortalEventPlayers.remove(event.getPlayer())) {
    ...
    @Template.Optional
    @Template.ImportList(value={@Template.Import(value="net.minecraft.core.BlockPos"), @Template.Import(value="net.minecraft.server.level.ServerPlayer"), @Template.Import(value="net.minecraft.server.level.ServerLevel"), @Template.Import(value="net.minecraft.network.protocol.Packet"), @Template.Import(value="net.minecraft.network.protocol.game.ClientboundGameEventPacket")})
